...
function write(filter: any, firstCall?: boolean): any {
  switch (filter.type) {
    // spatialClass
    case 'BBOX':
...
//         !cqlAst.filters.find((childFilter) => {
//           return childFilter.negated !== parentNode.negated
//         }))

/**
 * Essentially we want to turn all group negations into individual negations to aid simplification
 * @param cqlAst
 */
function applyDeMorganTheorem(
  cqlAst: FilterBuilderClass | FilterClass
): FilterBuilderClass | FilterClass {
  if (isFilterBuilderClass(cqlAst) && cqlAst.negated) {
    return new FilterBuilderClass({
      negated: false,
      filters: cqlAst.filters.map((filter) => {
        const demorganedFilter = applyDeMorganTheorem(filter)
        if (isFilterBuilderClass(demorganedFilter)) {
          return new FilterBuilderClass({
            ...demorganedFilter,
            negated: !demorganedFilter.negated,
          })
        } else {
          return new FilterClass({
            ...demorganedFilter,
...
export default {
  ...
  write(filter: FilterBuilderClass): string {
    try {
      ...
      return write(standardCqlAst, true)
    } catch (err) {
      console.log(err)
      return write(
