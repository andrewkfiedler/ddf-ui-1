...
class RenderNode {
  ...
  add(newRenderNode: RenderNode) {
    if (this.next) {
      this.next.add(newRenderNode)
    } else {
      newRenderNode.prev = this
      this.next = newRenderNode
    }
  }
  constructor({ render }: { render: () => void }) {
    this.render = render
  }
}

let rootRenderNode = undefined as undefined | RenderNode

const addToQueue = (renderNode: RenderNode) => {
  if (rootRenderNode) {
    rootRenderNode.add(renderNode)
  } else {
...
const processQueue = () => {
  ...
  setTimeout(() => {
    ...
    if (rootRenderNode !== undefined) {
      rootRenderNode.process()
      processQueue()
    }
  }, 30)
}
...
export const ResultItem = ({
  ...
  // }, [])
  // if (firstRender) {
  //   return <Skeleton variant="text" height={75} />
  // }
